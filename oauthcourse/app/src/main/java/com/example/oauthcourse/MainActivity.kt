package com.example.oauthcourse
...
import com.example.oauthcourse.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        account = Auth0(applicationContext)
    ...
    private suspend fun loginWithBrowserCoroutines() {
        ...
        try {
            ...
            showSnackBar("Failure : ${authenticationException.message}")
            updateUI()
        }
    }
        ...
        WebAuthProvider.login(account).withScheme(getString(R.string.com_auth0_scheme))
            .start(this, object : Callback<Credentials, AuthenticationException> {
                override fun onFailure(error: AuthenticationException) {
                    showSnackBar("Failure: ${error.message}")
                    updateUI()
                }
